...
页面不得根据分子式、原子排列顺序或视觉便利自行猜测化学键。离子晶体和固体没有合适的离散分子拓扑时，只能以无键线的“配方单元组成示意”展示。
...
上述限制适用于 `molecular-structures.v1.json`。`element-compounds.v1.json` 可以记录离子盐与固体，但必须标记为 `formula-unit / composition-schematic`，并保持 `bonds: []`；宁可只画原子组成，也不得用虚构连接线补齐。
...
生成过程遵守 PubChem PUG REST 每秒不超过 5 次请求的规则。校验器锁定乙炔 `H—C≡C—H` 拓扑，检查全部记录的分子式、连通性、键索引、键级和来源 URL，并保证原子序数 1—80 的每一种元素至少有一条结构记录。
