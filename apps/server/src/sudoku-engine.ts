export type ThemeId = "gems" | "elements" | "numbers" | "letters" | "crew";
export const THEME_IDS = ["gems", "elements", "numbers", "letters", "crew"] as const;
export type SudokuSpec = { name: string; n: number; h: number; w: number; clues: number; reward: number };
export type Cell = { value: number; crossed: number[]; noted: boolean };
export type Story = { title: string; teaser: string; rows: string[][] };
export type Puzzle = { given: number[]; solution: number[]; algorithm: "随机回溯" | "结构置换"; seed: number; level: number };
export type Game = Puzzle & { id: string; theme: ThemeId; createdAt: string; updatedAt: string; completedAt: string | null; story: Story; cells: Cell[]; undo: Cell[][]; hints: number };
export type CellAction = { type: "set" | "cross"; index: number; value: number } | { type: "note" | "clear" | "hint"; index: number } | { type: "undo" };
export function rewardForLevel(level: number): number {
  if (!Number.isInteger(level) || level < 0 || level > 5) throw new Error("请选择六档难度之一");
  return 30 + 20 * level;
}
export const LEVELS: readonly SudokuSpec[] = [
  { name: '星芽启程', n: 4, h: 2, w: 2, clues: 10, reward: 30 },
  { name: '月光寻路', n: 4, h: 2, w: 2, clues: 7, reward: 50 },
  { name: '彗星探险', n: 6, h: 2, w: 3, clues: 23, reward: 70 },
  { name: '星云解谜', n: 6, h: 2, w: 3, clues: 16, reward: 90 },
  { name: '银河领航', n: 9, h: 3, w: 3, clues: 42, reward: 110 },
  { name: '宇宙织梦', n: 9, h: 3, w: 3, clues: 30, reward: 130 },
];
export const THEMES = [
...
export function shuffle<T>(list: readonly T[], random: () => number) { const a = [...list]; for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(random() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; } return a; }
export function peers(i: number, spec: SudokuSpec) {
...
export function choices(board: readonly number[], i: number, spec: SudokuSpec) { const used = new Set(peers(i, spec).map(j => board[j])); return range(spec.n).map(v => v + 1).filter(v => !used.has(v)); }
export function solve(input: readonly number[], spec: SudokuSpec, limit = 2, random?: () => number): { count: number; first: number[] | null } {
  const b = [...input], allPeers = range(b.length).map(i => peers(i, spec)); let count = 0; let first: number[] | null = null;
  if (b.length !== spec.n ** 2 || b.some((v, i) => !Number.isInteger(v) || v < 0 || v > spec.n || v && allPeers[i].some(j => b[j] === v))) return { count: 0, first: null };
  function visit() {
    let best = -1; let opts: number[] = [];
...
function patternSolution(spec: SudokuSpec, random: () => number) {
  const { n, h, w } = spec;
...
export function generateStory(themeId: ThemeId, n: number, seed: number): Story {
  const theme = THEMES.find(t => t.id === themeId); if (!theme) throw new Error('请选择一个故事世界');
  const random = rng(seed ^ 0xABCDEF), pick = <T,>(a: T[]): T => a[Math.floor(random() * a.length)];
...
export function makeGame(level: number, theme: ThemeId, seed: number, id: string, now = new Date().toISOString()): Game {
...
export function act(game: Game, action: CellAction): { completed: boolean; message: string; hintValues?: number[] } {
  ...
  if ((action.type === 'cross' || action.type === 'set') && (!Number.isInteger(action.value) || action.value < 1 || action.value > spec.n)) throw new Error('请使用本局图例里的内容');
  let message = '';
...
export function publicGame(game: Game | null) {
  if (!game) return null;
  const { solution, undo, story, seed: _seed, ...rest } = game, { n } = LEVELS[game.level];
...
export type PublicGame = NonNullable<ReturnType<typeof publicGame>>;
