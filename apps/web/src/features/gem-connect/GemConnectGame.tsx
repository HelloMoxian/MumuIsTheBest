...
import { formatTime, GEMS, LEVELS, rankRecords, type Completion, type RecordEntry } from "./logic";
...
export function GemConnectGame() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (next.phase === "complete" && next.level < 10) {
        ...
        boardScroll.current?.scrollTo({ left: 0, top: 0 });
      }
    }, 80);
    const hidden = () => {
  ...
  useEffect(() => {
    if (!game.hint.length || game.phase !== "playing") return;
    const container = boardScroll.current, tile = tilesRef.current[game.hint[0]];
    if (!container || !tile) return;
    const bounds = container.getBoundingClientRect(), position = tile.getBoundingClientRect();
    // Scroll the board only, so an off-screen hint is visible without moving the whole page.
    if (position.top < bounds.top || position.bottom > bounds.bottom || position.left < bounds.left || position.right > bounds.right) {
      container.scrollTo({ top: container.scrollTop + position.top - bounds.top - bounds.height / 3,
        left: container.scrollLeft + position.left - bounds.left - bounds.width / 3,
        behavior: reduced.current ? "instant" : "smooth" });
    }
  }, [game.hint, game.phase]);
  ...
  function moveFocus(event: KeyboardEvent, index: number) {
    const offsets: Record<string, number> = { ArrowRight: 1, ArrowLeft: -1, ArrowDown: config.cols, ArrowUp: -config.cols };
    const offset = offsets[event.key]; if (!offset) return;
    event.preventDefault();
    for (let next = index + offset; next >= 0 && next < game.board.tiles.length; next += offset) {
      if (Math.abs(offset) === 1 && Math.floor(next / config.cols) !== Math.floor(index / config.cols)) break;
      if (game.board.tiles[next] !== null) { tilesRef.current[next]?.focus(); return; }
    }
  }
  return <main className="gc-page">
    ...
    <nav className="gc-levels" aria-label="十关星光航线">{LEVELS.map((level, index) => {
      const completed = records.some(record => record.rulesVersion === 2 && record.level === index + 1) || pending.some(record => record.level === index + 1);
    ...
    <section className="gc-play" aria-label="宝石棋盘">
      <div className="gc-heading"><div className="gc-title-line"><h1>{config.name}</h1><span>第 {game.level} 关 · {total * 2} 颗</span></div>
      ...
      <div className={`gc-stage ${paused ? "gc-is-paused" : ""}`}><div className="gc-board-scroll" ref={boardScroll}>
        <div className={`gc-board-frame ${entryActive ? "gc-entering" : ""}`} style={{ "--gc-cols": config.cols, "--gc-rows": config.rows } as CSSProperties}>
          <div className={`gc-board ${paused ? "gc-concealed" : ""}`} key={game.id + ":" + game.entrance}>
            {game.board.tiles.map((tile, index) => {
              ...
              return <div className="gc-cell" key={index} style={{ "--gc-delay": entryDelay(index, config.rows, config.cols) + "ms" } as CSSProperties}>{kind !== undefined && kind !== null && <button
                ...
                aria-label={`第 ${Math.floor(index / config.cols) + 1} 行第 ${index % config.cols + 1} 列，${GEMS[kind].name}${game.hint.includes(index) ? "，提示宝石" : ""}`}
          ...
          {game.matches.length > 0 && <svg className={`gc-path ${paused ? "gc-concealed" : ""}`} viewBox={`0 0 ${config.cols + 1} ${config.rows + 1}`} preserveAspectRatio="none" aria-hidden="true">
            {game.matches.map(match => <polyline key={game.id + ":" + game.entrance + ":" + match.a + ":" + match.b} pathLength="1" points={match.path.map(point => `${point.c + 1},${point.r + 1}`).join(" ")} />)}
          </svg>}
        </div>
      ...
      <div className="gc-bottom-line"><p className="gc-feedback" role="status">{game.message}</p><button className="gc-button" onClick={() => setShowTime(value => !value)}>{showTime ? "隐藏计时" : "显示计时"}</button></div>
    </section>
    ...
    <details className="gc-records"><summary>本关星光榜 {best ? "· 最快 " + preciseTime(best.durationMs) : ""}</summary>
      <p className="gc-ranking-note">第 {game.level} 关 · 新版棋盘 · 按用时从短到长</p>
      ...
      <p className="gc-ranking-note">暂停、入场及无法操作的动画收尾不计时。连续配对期间正常计时。提示和重排不加罚时。旧版小棋盘成绩保留，不参与新版排名。</p>
    </details>
  </main>;
}
