...
2. 顶部粘贴阿里云 API Key；默认 WebSocket 地址已是当前北京业务空间地址，可按需要修改为同一规则下的其他阿里云业务空间地址。
3. 点击“保存本机配置”。随后重新打开页面也会自动使用这台电脑上的配置，页面不会显示 Key 明文。
4. 打开“功能测试” → “语音识别测试”，或滚动至“语音识别实验室”。
...
- 每一次 Fun-ASR 实时会话默认、且必须限制为最多 **2 分钟（120 秒）**。除非未来需求明确批准并同步修改本规范，否则所有接入 ASR 的算术、游戏和知识模块都继承这一上限。
- Fastify 服务端从收到阿里云 `task-started` 起计时；达到 120 秒后主动停止转发音频并发送 `finish-task`。这是权威限制，不能仅依赖浏览器计时器。
