...
- `learning/games/workout.json`：schemaVersion 1，稳定 ID `kids-workout`，动作池配置及每次课程的计划、前台进度、创建/更新时间。缺失时默认十个非跳跃动作，损坏/未来版本拒绝覆盖，串行原子保存。
- 能量币钱包兼容新增默认 0 的 `workoutRewardTotal`。完整五分钟（含休息）先保存资格，再按累计值入账每局 200 能量币；重试与重启补发不重复奖励。
